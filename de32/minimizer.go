...
func NewMinimizer(cost Cost, n int, min, max matrix32.Dense) *Minimizer {
	if n < 4 {
...
func (m *Minimizer) NextGen() (minCost, maxCost float32) {
	minCost = math.MaxFloat32
	maxCost = -math.MaxFloat32

	// Perform crossover
	for i, x := range m.Pop {
		f := 0.5 + m.rnd.Float32()*0.5
		a, b, c := m.abc(i)
